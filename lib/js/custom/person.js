function Person(options){
   ...
   this.marker = L.animatedMarker( L.polyline([t.loc]).getLatLngs(),{
      icon: t.icon,
      clickable: true,
      //title: getFullName(),
      autoStart: false,
      //distance: t.speed*0.00044704, // Convert to meters per millisec
      distance: 0.1,
      interval: 1,
      onEnd: function(){
         //t.marker.start();
   ...
   this.getTabbedDiv = function(){
      ...
      $('#stats').append(t.getStats());
      tabDiv.append('<div id="skills"></div>');
      $('#skills').append(t.getSkills());
      tabDiv.append('<div id="inventory"></div>');
      $('#inventory').append(t.getInventory());
